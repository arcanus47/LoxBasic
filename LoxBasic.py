...
class RTError(Error):
  ...
  def generate_traceback(self):
    ...
    while ctx:
      result = f'  Archivo {pos.fn}, línea {str(pos.ln + 1)}, en {ctx.display_name}\n' + result
...
class Parser:
  ...
  def statement(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Esperando 'RETURN', 'CONTINUE', 'BREAK', 'VAR', 'IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
      ))
  ...
  def expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Esperando 'VAR', 'IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
      ))
  ...
  def comp_expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Esperando int, float, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUN' or 'NOT'"
      ))
  ...
  def atom(self):
    ...
    return res.failure(InvalidSyntaxError(
      ...
      "Esperando int, float, identifier, '+', '-', '(', '[', IF', 'FOR', 'WHILE', 'FUN'"
    ))
...
class BaseFunction(Value):
  ...
  def check_args(self, arg_names, args):
    ...
    if len(args) < len(arg_names):
      return res.failure(RTError(
        self.pos_start, self.pos_end,
        f"{len(arg_names) - len(args)} Muy pocas argumentos pasaron a {self}",
        self.context
      ))
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_run(self, exec_ctx):
    ...
    try:
      ...
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        f"Error al cargar la secuencia de comandos \"{fn}\"\n" + str(e),
        exec_ctx
      ))
    ...
    if error:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        f"No se pudo terminar de ejecutar el script \"{fn}\"\n" +
        error.as_string(),
        exec_ctx
      ))
...
while True:
	...
	result, error = LoxBasic.run('<stdin>', text)
